...
jQuery(document).ready(function() { 
		...
		$('.gallery').each(function () { 
			...
			$(this).find('a').each(function () { 
				...
				if (imgFormat.length <= 5) {
					$(this).attr('data-rel', 'ilightbox[wp_gal_' + galUniqID + ']');
				}
			} );
	...
	(function ($) { 
		...
		$('[data-rel^="ilightbox["]').each(function () { 
			var item_rel = $(this).attr('data-rel');
		...
		$.each(gallery_array, function (gallery_array, gallery_id) { 
			$('[data-rel="' + gallery_id + '"]').iLightBox(ilightbox_settings)
		} );
	} )(jQuery);
	...
	(function ($) { 
		$(window).load(function () { 
			...
			$('a.ls-l, .hash-link a, a.hash-link').click(function () { 
